...
export function getSessionAnimalIndex() { return diskState.animalIndex ?? 0; }
export function getSessionPaletteIndex() { return diskState.paletteIndex ?? 0; }
export function animTick() { return Math.floor(Date.now() / 1500); }
export function moodTick() { return Math.floor(Date.now() / 10000); }
...
const ctxHistory = [];
export function recordContextPercent(pct) {
    const now = Date.now();
    // Don't record too frequently (min 1s gap)
    if (ctxHistory.length > 0 && now - ctxHistory[ctxHistory.length - 1].t < 1000)
        return;
    ctxHistory.push({ pct, t: now });
    if (ctxHistory.length > CTX_HISTORY_SIZE)
        ctxHistory.shift();
}
/** Returns context velocity in %/min. Positive = growing. */
export function getContextVelocity() {
    if (ctxHistory.length < 2)
        return 0;
...
export function getRelationshipTier() {
    const m = getMemory();
    if (m.totalSessions >= 50)
        return 'bestie';
    if (m.totalSessions >= 15)
        return 'friend';
