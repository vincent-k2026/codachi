...
export function render(input) {
    const { contextPercent, modelName, animalType, colors, git, fiveHourUsage, sevenDayUsage, frameIndex, } = input;
    ...
    if (git) {
        ...
        const stats = [];
        if (git.modified > 0)
            stats.push(`${rgb(255, 200, 50)}~${git.modified}${RESET}`);
        if (git.added > 0)
            stats.push(`${rgb(80, 220, 120)}+${git.added}${RESET}`);
        if (git.deleted > 0)
            stats.push(`${rgb(255, 80, 80)}-${git.deleted}${RESET}`);
        if (git.untracked > 0)
            stats.push(`${DIM}?${git.untracked}${RESET}`);
        if (stats.length > 0) {
            gitLine += ' ' + stats.join(' ');
        }
        if (git.ahead > 0)
            gitLine += ` ${DIM}ahead ${git.ahead}${RESET}`;
        if (git.behind > 0)
            gitLine += ` ${DIM}behind ${git.behind}${RESET}`;
