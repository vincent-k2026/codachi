...
export function render(input) {
    const { contextPercent, modelName, animalType, colors, git, fiveHourUsage, sevenDayUsage, animTick, moodTick, } = input;
    const termWidth = getTerminalWidth();
    const size = getBodySize(contextPercent);
    const animation = getAnimation(contextPercent, false);
    ...
    if (!git) {
        ...
        const fs = [];
        if (git.modified > 0)
            fs.push(`${rgb(255, 200, 50)}~${git.modified}${RESET}`);
        if (git.added > 0)
            fs.push(`${rgb(80, 220, 120)}+${git.added}${RESET}`);
        if (git.deleted > 0)
            fs.push(`${rgb(255, 80, 80)}-${git.deleted}${RESET}`);
        if (git.untracked > 0)
            fs.push(`${rgb(180, 180, 200)}?${git.untracked}${RESET}`);
        if (fs.length > 0)
            line2 += ` ${fs.join(' ')}`;
        if (git.insertions > 0 || git.deletions > 0) {
            const lp = [];
            if (git.insertions > 0)
                lp.push(`${rgb(80, 220, 120)}+${git.insertions}${RESET}`);
            if (git.deletions > 0)
                lp.push(`${rgb(255, 80, 80)}-${git.deletions}${RESET}`);
            line2 += ` ${SEP} ${lp.join(' ')} ${C}lines${RESET}`;
        }
        if (git.ahead > 0 || git.behind > 0) {
            const rp = [];
            if (git.ahead > 0)
                rp.push(`${rgb(255, 200, 50)}up${git.ahead}${RESET}`);
            if (git.behind > 0)
                rp.push(`${rgb(200, 100, 255)}dn${git.behind}${RESET}`);
            line2 += ` ${SEP} ${rp.join(' ')}`;
        }
        if (git.lastCommit)
